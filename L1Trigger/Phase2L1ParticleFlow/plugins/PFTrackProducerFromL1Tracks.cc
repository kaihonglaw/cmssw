...
namespace l1tpf {
  ...
  private:
    edm::EDGetTokenT<edm::RefVector<std::vector<l1t::PFTrack::L1TTTrackType>>> TrackTag_;
...
l1tpf::PFTrackProducerFromL1Tracks::PFTrackProducerFromL1Tracks(const edm::ParameterSet &iConfig)
    : TrackTag_(consumes<edm::RefVector<std::vector<l1t::PFTrack::L1TTTrackType>>>(iConfig.getParameter<edm::InputTag>("L1TrackTag"))),
...
void l1tpf::PFTrackProducerFromL1Tracks::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  edm::Handle<edm::RefVector<std::vector<l1t::PFTrack::L1TTTrackType>>> htracks;
  iEvent.getByToken(TrackTag_, htracks);
  //const auto &tracks = *htracks;
  for (const auto& tk : *htracks) {
  ...
  //for (unsigned int i = 0, n = tracks.size(); i < n; ++i) {
    ...
    std::ofstream PFcheck("PFcheck_new3.txt", std::ios::app);
    ...
    int quality = 0;
    for (int iq = 0, n = qualityBitSetters_.size(), bit = 1; iq < n; ++iq, bit <<= 1) {
      if (qualityBitSetters_[iq](*tk))
        quality += bit;
    }
