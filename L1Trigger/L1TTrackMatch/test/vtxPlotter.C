...
void Plotter(TString type1, TString type2) {
  ...
  TH2F* h_NN_err_vs_PV = new TH2F(
      "NN_res_vs_PV", ";True PV z_{0} [cm];True PV z_{0} - Reco PV z_{0} NN [cm];#Events", 60, -15, 15, 60, -15, 15);
  TH2F* h_FH_err_vs_PV = new TH2F(
      "FH_res_vs_PV", ";True PV z_{0} [cm];True PV z_{0} - Reco PV z_{0} FH [cm];#Events", 60, -15, 15, 60, -15, 15);

  TH2F* h_NN_vs_PV =
      new TH2F("NN_vs_PV", ";True PV z_{0} [cm];Reco PV z_{0} NN [cm];#Events", 60, -15, 15, 60, -15, 15);

  TH2F* h_FH_vs_PV =
      new TH2F("FH_vs_PV", ";True PV z_{0} [cm];Reco PV z_{0} FH [cm];#Events", 60, -15, 15, 60, -15, 15);

  int num_thresholds = 100;

  TH2F* h_FH_vtx_efficiency =
      new TH2F("h_FH_vtx_efficiency", ";Threshold;True PV z_{0} [cm];#Events", num_thresholds, 0, 1, 50, -15, 15);

  TH2F* h_NN_vtx_efficiency =
      new TH2F("h_NN_vtx_efficiency", ";Threshold;True PV z_{0} [cm];#Events", num_thresholds, 0, 1, 50, -15, 15);
...
vector<TVectorT<double>> CalcROC(TH1F* XHist, TH1F* YHist) {
  ...
  for (int i = 0; i < (num_threshold - 1); i++) {
    ...
    AUC_ROC[0] += (XHist->GetBinContent(i + 1) - XHist->GetBinContent(i)) *
                  (0.5 * (YHist->GetBinContent(i) + YHist->GetBinContent(i + 1)));
